...
int main() {
    ...
    int r = 0;
    rep(i,Q){
        int w = wvi[i].first.first;
        int v = wvi[i].first.second;
        int idx = wvi[i].second;
        while(r < M && w < yab[r].first){
            int a = yab[r].second.first;
            int b = yab[r].second.second;
            uni.unite(a,b);
            r++;
        }
        ans[idx] = uni.size(v);
