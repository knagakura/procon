...
struct UnionFind{
    ...
    ll size(int x){ return sizes[root(x)]; }
    int group_num(){
        set<int> s;
        for(int i = 0; i < n; ++i){
            s.insert(root(i));
...
int main() {
    int N;
    cin >> N;
    vector<pair<ll, pair<int, int>>> w_uv;
    rep(i,N-1){
        int u, v;
        ll w;
        cin >> u >> v >> w;
        u--, v--;
        w_uv.push_back({w, {u, v}});
    }
    sort(all(w_uv));
    UnionFind T(N);
    ll ans = 0;
    rep(i,N-1){
        ll w = w_uv[i].first;
        auto [u, v] = w_uv[i].second;
        dump(w, u, v);
        dump(T.size(u), T.size(v));
        ans += T.size(u) * T.size(v) * w;
        T.unite(u, v);
    }
    cout << ans << endl;
}
