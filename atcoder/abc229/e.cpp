...
struct UnionFind {
    ...
    void unite(int x, int y) {
        added[x] = 1;
        added[y] = 1;
        int preParentX = Parent[x];
        int preParentY = Parent[y];
        x = root(x);
        y = root(y);
        if(x == y)
            return;
        if(sizes[x] < sizes[y]) {
            swap(x, y);
            swap(preParentX, preParentY);
        }
        Parent[y] = x;
        sizes[x] += sizes[y];
        if(preParentX == x && Parent[x] != x) {
            depSize--;
        }
        if(preParentY == y && Parent[y] != y) {
            depSize--;
        }
    }
