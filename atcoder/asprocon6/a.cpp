...
#define myprint(v) { cerr<<#v<<": [ "; for(auto _ : v) cerr<<_<<", "; cerr<<"]"<<endl; }
...
auto solve(const Input& input) {
    ...
    for (const auto& o : input.order) {
        ...
        // パン焼き機は、0からじゃなくてもいい
        vector<bool> ok(input.header.M, false);
        auto start = XorShift() % input.header.M;
        ok[start] = true;

        auto pair = find_next_m_and_t(start, o.i, input);
