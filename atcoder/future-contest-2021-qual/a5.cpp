...
CardTour chokudai_search(const int& n_itr = 100,  int chokudai_width = 1, const int MaxCnt = 1000, bool isDump = 1){
    vector<priority_queue<CardTour>> ques(cardN+1);
    vector<int> ques_size(cardN+1, 0);
    dump(CardTour().seq);
    ...
    rep(_, n_itr){
        ...
        for(int depth = 0; depth < cardN; depth++){
            ...
            ques_size[depth] = que.size();
            ques_size[depth+1] = nxt_que.size();
            chokudai_width = max(1, 50 - (cardN-depth-1));
            if(_ == 0){
                dump(depth, chokudai_width);
            }
            for(int i = 0; i < chokudai_width; i++){
                ...
                for(int idx: aStage.nearCards[from]){
                    ...
                    if(not state.used[idx]){
                        ...
                        // if(mp.count(nxt_state.seq))continue;
                        mp[nxt_state.seq]++;
...
void solve_chokudai(){
    dump("chokudai");
    // 順番を決めて
    CardTour best_state = chokudai_search();
    dump(best_state.seq);
