...
struct Player{
    ...
    void TourAll(const vector<pair<int,int>> &seq2){
        assert(seq2.size() == 2*cardN+1);
        for(auto p: seq2){
            if(p.second){
                goOut(aStage.getSquarePos(p.first));
            }
            else{
                goIn(aStage.getCardPos(p.first));
            }
        }
    }
    ...
    void goOut(const pair<int, int>& p){
        goOut(p.first, p.second);
    }
...
struct CardTour{
    ...
    void add2Seq(const int idx){
        ...
        used[idx] = true;
        nowx = aStage.getCardPos(idx).first;
        nowy = aStage.getCardPos(idx).second;
        seq2.push_back({idx, 0});
        return;
    }
    void pushCard(const int idx){
        ...
        // }
        int preidx = seq.back();
        cost += dist({nowx, nowy}, aStage.getCardPos(idx));
        add2Seq(idx);
        // 今入れたのが最後である場合
        if(seq.size() == cardN+1){
    ...
    void popCard(){
        ...
        cost += dist(aStage.getSquarePos(seq.back()), make_pair(nowx, nowy));
        nowx = aStage.getSquarePos(seq.back()).first;
        nowy = aStage.getSquarePos(seq.back()).second;
        seq.pop_back();
        seq2.push_back({seq.back(), 1});
    }
...
CardTour chokudai_search2(const int& n_itr, const int& chokudai_width = 1, bool isDump = 1){
    vector<priority_queue<CardTour>> ques(2*cardN+1);
    dump(CardTour().seq);
    ques[0].push(CardTour()); // デフォルトのやつを挿入 初期位置のみ入ってる
    rep(_, n_itr){
        if(isDump && _%20 == 0){
        ...
        for(int depth = 0; depth < 2*cardN; depth++){
            auto &que = ques[depth];
            auto &nxt_que = ques[depth + 1];
            for(int i = 0; i < chokudai_width; i++){
                if(que.empty())break;
    ...
    dump("end");
    assert(not ques[2*cardN].empty());
    CardTour res = ques[2*cardN].top();
    dump(res.seq);
    return res;
}
void input(){
...
void solve_chokudai(){
    ...
    dump(best_state.seq);
    dump(best_state.cost);
    aPlayer.getCardbySeq(best_state.seq);
    aPlayer.hidariUekaraOku();
    aPlayer.hidariUekaraToru();
}

void solve_mazechokudai(){
    CardTour best_state = chokudai_search2(100, 1);
    aPlayer.TourAll(best_state.seq2);
    aPlayer.hidariUekaraToru();
}
int main() {
    MyTimer timer;
