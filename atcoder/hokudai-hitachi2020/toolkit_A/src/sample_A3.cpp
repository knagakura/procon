...
struct strategy :public P {
    ...
    virtual void command(const grid_info& g_i, const EV_info& ev_i, const order_info& order_i, int large_charge, int emergency_charge){
...
struct my_all_stay : strategy<P> {
    ...
    void command(const grid_info& grid_i, const EV_info& ev_i, const order_info&, int large_charge, int emergency_charge) {
        ...
        for (size_t n = 0; n < ev_i.N_EV; ++n) {
            ...
            if (current != pos) {
                ...
                if(ev_i.c[n].charge >= large_charge){
                ...
                else if(grid_i.power[pos] >= 30000){
                        S::enqueue(n, strprintf(
                                "charge_from_grid % zu",
                                min(15000 - ev_i.c[n].charge, S::EV.V_EV_max)),
                                   1);
                }
...
//         str.reset(new my_random_walk<A>(prob, gs));
        ...
        for(size_t t = 0; t < prob.T_max; ++t){
            ...
            ev_i.load(cin);
            int large_charge = 15000;
            while(large_charge > 3000) {
                str->command(grid_i, ev_i, order_i, large_charge, 1000);
