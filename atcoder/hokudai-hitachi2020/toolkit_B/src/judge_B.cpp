...
#include "judge_B.lib/testlib.hpp"
#include<iostream>
#include<vector>
#include<time.h>
...
bool judge::valid_input(){//contestant's input
	...
	for(size_t vehicle_id = 0; vehicle_id < N_vehicles; ++vehicle_id){
		...
		if(s == "stay"){
			...
			inf.readSpace();
			int ord_id = inf.readInt();
			--ord_id;
			ensure(valid_pickup(ord_id,vehicle_id));
...
bool judge::next_time_step(){
	//ordersのサイズがどちらでも大丈夫そう
	if( T_now < orders.size() ){//Orderの発生の確認
		num_orders += orders[T_now].size();//荷物の数を増やす
