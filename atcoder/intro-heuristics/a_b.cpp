...
vector<ll> calc(vector<int> &v, int ch_idx, int pre_fix_score, vector<ll>&last_vec){
    vector<ll> scores(D+1,0);
    ...
    for(ll d = ch_idx; d < D; d++){
        ...
        scores[d+1] += s[d][v[d]];
        rep(i,26){
            if(v[d] == i)last_vec[i] = d+1;
            scores[d+1] -= c[i] * ((d+1) - last_vec[i]);
        }
    }
    return scores;
}
void simulate(){
    ...
    while(1){
        ...
        nxt[chidx] = chtype;
        auto nxt_last = last;
        auto nxt_scores = calc(nxt, chidx, pre_scores[chidx], last);
