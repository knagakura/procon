...
int main() {
    ...
    int N, M;
    cin >> N >> M;
    vec<double> x(N+M),y(N+M);
    vec<int> c(N+M);
    rep(i,N)cin >> x[i] >> y[i] >> c[i];
    rep(j,M)cin >> x[N+j] >> y[N+j] >> c[N+j];
    auto dist = [&](int i, int j) -> double{
        double tw = (x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j]);
        double res = sqrt(tw) * ((c[i] != c[j]) ? 10: 1);
        return res;
    };
    ...
    for(int bits = 0; bits < 1<<M; bits++){
        ...
        rep(j,M){
            if(bits & bit(j))v.emplace_back(N+j);
        }
        int sz = v.size();
        vec<pair<double, i_i>> edges;
        rep(j,sz)rep(i,j){
            edges.push_back({dist(v[i],v[j]),{v[i],v[j]}});
        }
        sort(all(edges));
        double tmp = 0;
        /*calc*/
