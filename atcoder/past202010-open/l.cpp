    ...
    while(Q--){
        ...
        if(t == 3){
            ...
            if(0 <= u-1){
                {
                    ll dh = abs(h[u]-h[u-1]);
                    if(h[u-1] < h[u])mp[(u-1)%2][dh] -= 1;
                    else mp[u%2][dh] -= 1;
                }
                {
                    ll dh = abs(h[u]+v-h[u-1]);
                    if(h[u-1] < h[u]+v)mp[(u-1)%2][dh] += 1;
                    else mp[u%2][dh] += 1;
                }
            }
            if(u+1 < N){
                {
                    ll dh = abs(h[u+1]-h[u]);
                    if(h[u] < h[u+1])mp[u%2][dh] -= 1;
                    else mp[(u+1)%2][dh] -= 1;
                }
                {
                    ll dh = abs(h[u+1]-h[u]-v);
                    if(h[u]+v < h[u+1])mp[u%2][dh] += 1;
                    else mp[(u+1)%2][dh] += 1;
                }
            }
            h[u] += v;
        }
        cout << mp[0][cnt[0]-cnt[1]] + mp[1][cnt[1]-cnt[0]] << endl;
    }
}
