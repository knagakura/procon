...
long double cal(ll mid) {
	//cerr << mid << endl;
	//debugs(c*(mid), h * (mid + 1), (2 * mid + 1));
	//cerr<< (double)(c*(mid) + h * (mid + 1)) / (2 * mid + 1)  <<endl;
	return (long double)(c*(mid) + h * (mid + 1)) / (2 * mid + 1);
}
...
int main() {
	...
	while (Q--) {
		...
		//debug(hi);
		//debug(low);
		if (abs(cal(low) - t) < abs(cal(hi) - t)) hi = low;
		cout << hi * 2 + 1 << endl;
	}
	//*/
