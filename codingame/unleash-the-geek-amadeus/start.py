...
while True:
    ...
    # radar_cooldown: turns left until a new rafdar can be requested
    ...
    for i in range(len(game.my_robots)):
        ...
        #print("Debug messages...", game.my_robots[i].x, game.my_robots[i].y,file=sys.stderr)
        x = game.my_robots[i].x
        y = game.my_robots[i].y
        flag = True
        while(flag):
            nx = random.randrange(x-4, x + 5, 1)
            ny = random.randrange(y-4, y + 5, 1)
            if(abs(nx-x) + abs(ny-y)<=4 and 0<=nx<width and 0<=ny<height):
                game.my_robots[i].move(nx,ny)
                flag = False
