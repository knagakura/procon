...
void Pac::move_com() {
    ...
    if (not SuperPellets.empty()) {
        ...
        if (maxsize > 0) {
            ...
            int to_idx = -1;
            print(for_dir_mp);
            rep(i, ThreeWalls_Used.size()) {
                if (ThreeWalls_Used[i])continue;
                int tmpx = ThreeWalls[i].second;
                int tmpy = ThreeWalls[i].first;
                if (visited[tmpy][tmpx])continue;
                int dist = abs(x - tmpx) + abs(y - tmpy);
                if (for_dir_mp[{tmpx, tmpy}])dist -= 100;
                if (chmin(min_dist, dist)) {
                    cerr << tmpx << " " << tmpy << " " << dist << endl;
                    to_idx = i;
                }
            }
            if (min_dist != INF) {
                cx = ThreeWalls[to_idx].second;
                cy = ThreeWalls[to_idx].first;
                ThreeWalls_Used[to_idx] = true;
            }
                //all_search
            else {
                auto p = all_search();
...
vector<string> Pac::action() {
    ...
    if (history[turn][id].x == history[turn - 1][id].x &&
...
void Pac::four_direction_search() {
    ...
    rep(j, 4) {
        ...
        while (true) {
            ...
            if (!visited[nh][nw])for_dir_mp[{nw, nh}]++;
...
void tmp_field_setting() {
    ...
    ThreeWalls_Used.assign(ThreeWalls.size(), false);
}
